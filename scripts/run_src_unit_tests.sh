...
REQUIRED_COMMANDS=("mypy" "ruff" "pytest")
...
ruff check --select S --ignore S311 "$SOURCE_DIR"

# --- Testing ---
print_header "Running Automated Unit Tests"
echo "Running pytest for unit tests and generating a code coverage report..."
pytest "$UNIT_TESTS_DIR/core/" --cov="$SOURCE_DIR" --cov-fail-under=95 --cov-report=term-missing
